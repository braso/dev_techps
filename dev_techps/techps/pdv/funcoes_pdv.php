	...
	if ( strtolower($a_prod[prod_tx_servico])!='sim' && ($a_prod[prod_tx_qtde]-$quantidade)<0 ) {
		...
		if ( trim($_POST[senha_solicitada])=='sim' ) {
			...
			if ( $_POST[senha_gerencial]=='' ) {
				...
				$sql_senha = query("SELECT * FROM user WHERE user_tx_status='ativo' AND (user_tx_nivel like '%Administrador%' OR user_tx_nivel='Gerente') AND user_tx_senha='".md5($_POST[senha_gerencial])."' LIMIT 1");
