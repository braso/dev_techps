<?php
	...
	function inserir($tabela,$campos,$valores){
		...
		try{
			query("INSERT INTO $tabela ($campos) VALUES($valores);") or die();
			$sql = query("SELECT LAST_INSERT_ID();") or die();
		}catch (Exception $e){
			return;
		}
	...
	function campo_jornada($nome,$variavel,$modificador,$tamanho){	
		...
		$campo='<div class="col-sm-'.$tamanho.' margin-bottom-5">
				<label><b>'.$nome.'</b></label>
				<input name="'.$variavel.'" id="'.$variavel.'" value="'.$modificador.'" autocomplete="off" type="text" class="form-control input-sm" >
			</div>';
	...
	function checkbox_banco($nome, $variavel, $modificadoRadio,$modificadoCampo, $modificadoCampo2, $tamanho) {
		...
		<div id="' . $variavel . '" class="col-sm-' . $tamanho . ' margin-bottom-5" style="display: none;">
				...
				<input class="form-control input-sm" type="number" value="'.$modificadoCampo.'" id="outroCampo" name="quantDias" autocomplete="off">
		</div>
		<div id="limiteHoras" class="col-sm-' . $tamanho . ' margin-bottom-5" style="display: none;">
			<label><b>Quantidade de Horas Limite:</b></label>
			<input class="form-control input-sm" type="number" value="'.$modificadoCampo2.'" id="outroCampo" name="quantHoras" autocomplete="off">
		</div>
		';
	...
	function campo($nome,$variavel,$modificador,$tamanho,$mascara='',$extra=''){
		...
		elseif($mascara=="MASCARA_HORA")
			$data_input="<script>$('[name=\"$variavel\"]').inputmask('99:99', { clearIncomplete: true, placeholder: \" \" });</script>";
	...
	function icone_excluir($id,$acao,$campos='',$valores='',$target='',$icone='',$msg='Deseja excluir o registro?',$title=''){
		...
		return "<center><a title=\"$title\" style='color:gray' onclick='javascript:contex_icone(\"$id\",\"$acao\",\"$campos\",\"$valores\",\"$target\",\"$msg\");' ><spam $icone></spam></a></center>";
