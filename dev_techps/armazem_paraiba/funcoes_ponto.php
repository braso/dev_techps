...
function diaDetalheEndosso($matricula, $data, $status = ''){
	...
	$sqlMotorista = query("SELECT e.*, ci.cida_nb_id, ci.cida_tx_uf,
	par.para_tx_acordo
	FROM entidade e
	INNER JOIN empresa em ON e.enti_nb_empresa = em.empr_nb_id
	INNER JOIN cidade ci ON ci.cida_nb_id = em.empr_nb_cidade
	INNER JOIN parametro par ON e.enti_nb_parametro = par.para_nb_id
	WHERE e.enti_tx_status != 'inativo' AND e.enti_tx_matricula = '$matricula'
	LIMIT 1");
	$aMotorista = mysqli_fetch_array($sqlMotorista,MYSQLI_ASSOC);
// 	$aMotorista = carrega_array($sqlMotorista);
	
	if ($aMotorista['cida_nb_id'] && $aMotorista['cida_tx_uf']) {
		$extraFeriado = " AND (feri_nb_cidade = '".$aMotorista['cida_nb_id']."' OR feri_tx_uf = '".$aMotorista['cida_tx_uf']."')";
	}

	$horaAlertaJornadaEfetiva = ($aMotorista['para_tx_acordo'] == 'Sim')? '12:00': '10:00';

	$sqlFeriado = "SELECT feri_tx_nome FROM feriado WHERE feri_tx_data LIKE '____-10-12%' AND feri_tx_status != 'inativo' AND (feri_nb_cidade = '2914604' OR feri_tx_uf = 'BA')";
	$queryFeriado = query($sqlFeriado);
	$stringFeriado = '';
