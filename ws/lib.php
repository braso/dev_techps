...
function get_user_jorneys($userid){
    ...
    foreach($data as $ponto){
        if($ponto["pont_tx_tipo"]%2){//inicio
            if($ponto["pont_tx_tipo"]==1){//Inicio Jornada
                if($currentJoeney){
                    $currentJoeney->breaks = array_merge($currentJoeney->breaks,array_values($currentBreak));
                    $currentBreak = [];
                }
                $currentJoeney= create_jorney_ob($ponto,"journey",$userid);
                $currentJoeney->finalDateTime = Null; 
            }
            else{
                if($currentBreak[$ponto["pont_tx_tipo"]]){
                    $currentJoeney->breaks[] = $currentBreak[$ponto["pont_tx_tipo"]];
                    $currentBreak[$ponto["pont_tx_tipo"]] = null;
                }
                $currentBreak[$ponto["pont_tx_tipo"]] = create_jorney_ob($ponto,"break",$userid,
            ...
            if($ponto["pont_tx_tipo"]==2){//Fim Jornada
                if(!$currentJoeney){
                    $currentJoeney= create_jorney_ob($ponto,"journey",$userid);
                    $currentJoeney->startDateTime = Null; 
                }
                $currentJoeney->breaks = array_merge($currentJoeney->breaks,array_values($currentBreak));
                $currentBreak = [];
                $currentJoeney->finalDateTime = $ponto["pont_tx_data"];
                $jorneysArray[] = $currentJoeney;
                $currentJoeney = null;
            }
            else{
                ...
                if($currentBreak[$tipo]){
                    ...
                    $currentJoeney->breaks[] = $currentBreak[$tipo];
                    $currentBreak[$tipo] = null;
                }
                $currentBreak[$tipo] = create_jorney_ob($ponto,"break",$userid,
                    trim(str_replace("inicio de ","",strtolower($auxArray[$tipo]))));
                $currentBreak[$tipo]->startDateTime = Null; 
            }
        }
    }
    if(count($currentBreak)>0 && !$currentJoeney){
        $currentJoeney = create_jorney_ob(["pont_nb_id"=>null,"pont_tx_data"=>null],"jorney",$userid);
    }
    if($currentJoeney){
        $currentJoeney->breaks = array_merge($currentJoeney->breaks,array_values($currentBreak));
        $jorneysArray[] = $currentJoeney;
...
//Jorney Functions
function create_jorney_ob($ponto,$type,$userid,$btype=''){
