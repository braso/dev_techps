document.addEventListener("DOMContentLoaded", () => {
    ...
    filterForm.addEventListener("submit", (event) => {
        ...
        axios
            .post("https://logsync.dev.enabled.app.br/data", {
                plate,
                date: formattedDate,
                speed,
            })
            .then((response) => {
                ...
                if (resultData.length === 0) {
                    resultsDiv.textContent =
                        "Nenhum resultado encontrado para a data e velocidade selecionadas.";
                } else {
                    displayResults(resultData, plate, date, speed); // Passa a data original
    ...
    function displayResults(data, plate, formattedDate, speed) {
    ...
    `;
        ...
        <h3><b>Resumo:</b> Consulta feita na Data - <b>${getCurrentDateTime()}</b> para a Placa <b>${plate}</b>, na data de <b>${formatDateBR(
            formattedDate
        )}</b> onde se constatou paradas:</h3>
        <p><i class="fas fa-power-off" style="color: green;"></i> <b>${formatTime(
            totalTrueTime
        )}</b> Com Ignição Ligada. Totalizando <b>${totalStopsIgnitionOn}</b> Paradas .</p>
        <p><i class="fas fa-power-off" style="color: red;"></i> <b>${formatTime(
            totalFalseTime
        )}</b> Com Ignição Desligada. Totalizando <b>${totalStopsIgnitionOff}</b> Paradas .</p>
    `;
        ...
        for (var i = 0; i < images.length; i++) {
            images[i].addEventListener("click", function () {
                ...
                function formatTime(timeString) {
                    if (!timeString || timeString === "undefined") return "";
                ...
                if (startTime === null) {
                ...
                } else if (startTime !== null && logoState) {
                    // Segunda seleção (fim)
                    var endTime = end;
                    ...
                    var allRows = document.querySelectorAll("table tr");
                    var startIndex = Array.from(allRows).indexOf(startRow);
                    var endIndex = Array.from(allRows).indexOf(row);

                    for (
                        var i = Math.min(startIndex, endIndex);
                        i <= Math.max(startIndex, endIndex);
                        i++
                    ) {
                        var currentRow = allRows[i];
                        if (currentRow !== startRow) {
                            // Não pinta a linha inicial
    ...
    function appendRow(tbody, row) {
        ...
        tr.innerHTML = `
        <td><img src="LGS.png" alt="Ícone"  class="row-img" /></td>
        <td>${moduleDate}</td>
        <td>${moduleTime}</td>
        <td>${row.endereco}</td>
        <td>${row.latitude}</td>
    ...
    function appendStopRow(
        ...
        if (row.ignition === "true" && parseInt(row.speed) <= 5) {
            tr.classList.add("high-speed");
        } else if (row.ignition === "false" && parseInt(row.speed) <= 5) {
            tr.classList.add("low-speed");
        } else if (parseInt(row.speed) > 5) {
        ...
        let hodometroDifference =
            previousHodometro !== null
                ? formatDistance((row.hodometro - previousHodometro).toFixed(2))
                : "<--"; // Se previousHodometro for null, a diferença não é aplicável

        tr.innerHTML = ` <td><img src="LGS.png" alt="Ícone"  class="row-img" /></td>
    ...
    <td> <a href="https://www.google.com/maps/place/${row.latitude},${
            row.longitude
        }" target="_blank">
   ...
   <td>${formatDistance(
       row.hodometro
   )}</td> <!-- Formatando a distância do hodômetro -->
