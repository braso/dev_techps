		...
		$(document).ready(function(){
			$('[id^=\"contex-grid-\"]').on('draw.dt', function(){
				...
				for (var r = 0; r < rows.length; r++) {
					...
					if (txt.length > 0) {
						var path = txt;
						tds[fotoIdx].innerHTML = '<img src=\"'+path+'\" style=\"width:35px;height:25px;object-fit:cover;border-radius:3px;cursor:pointer;border:1px solid #ccc;vertical-align:middle;\" onclick=\"event.preventDefault(); document.getElementById(\\'fotoModalImage\\').src = \\''+path+'\\'; document.getElementById(\\'fotoModal\\').style.display = \\'flex\\';\" title=\"Ver foto\">';
					}
				}
			});
		});
