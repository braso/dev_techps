	...
	function empresas($aEmpresa) {
		...
		echo 
			"<style>
				...
				tr.totais > th:nth-child(n+2)+tr.titulos > th:nth-child(-n+12){
					text-align: justify;
				}
				.titulos{
					background-color: #99ccff;
				}
				tr.titulos > th:nth-child(-n+12){
					text-align: justify;
				}
							...
							<thead>
								<tr class='totais'>
									...
									(($empresasTotais != null)? 
										...
										."<th colspan='1'>".(($empresasTotais["EmprTotalSaldoAnter"] == "00:00")? 	"": $empresasTotais["EmprTotalSaldoAnter"]).	"</th>"
						...
						$.ajax({
							...
							success: function(data){
								...
								$.each(data, function(index, item){
									...
									var porcentagem = ((item.endossados/item.totalMotorista) * 100)*(!isNaN(item.endossados) && !isNaN(item.totalMotorista) && item.totalMotorista !== 0);
