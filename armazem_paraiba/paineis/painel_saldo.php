    ...
    function criar_relatorio_saldo(){
        ...
        foreach ($empresas as $empresa){
            ...
            foreach ($motoristas as $motorista) {
                ...
                [
                    $totalJorPrevResut, $totalJorPrev, $totalJorEfe,
                    $totalHE50, $totalHE100,
                    $totalAdicNot, $totalEspInd,
                    $totalSaldoPeriodo, $totalSaldofinal, $saldoAnt
                ] = array_pad([], 10, "00:00");
        ...
        if (!empty($_POST["empresa"]) && !empty($_POST["busca_dataInicio"]) && !empty($_POST["busca_dataFim"])) {
            ...
            if (is_dir($pasta) != false){
                ...
                if (filemtime($pasta."/totalMotoristas.json") == $lastUpdateTimestamp) {
                    $emissao = date("d/m/Y H:i", $lastUpdateTimestamp); //Utilizado em painel_saldo_html.php
                }
                ...
                $performance = porcentagemSaldo(count($motoristas), $quantMeta, $quantNega, $quantPosi);

                $dataInicioFormatada = $dataTimeInicio->format('d/m'); //Utilizado em painel_saldo_html.php
                $dataFimFormatada = $dataTimeFim->format('d/m'); //Utilizado em painel_saldo_html.php

                include_once "painel_saldo_html.php";

                if(!empty($_POST["empresa"]) && !empty($_POST["busca_dataInicio"]) && !empty($_POST["busca_dataFim"])){
                    $valorTotais = [
                        $motoristasTotais["empresaNome"],
                        $motoristasTotais["jornadaPrevista"],
                        $motoristasTotais["JornadaEfetiva"],
                        $motoristasTotais["he50"],
                        $motoristasTotais["he100"],
                        $motoristasTotais["adicionalNoturno"],
                        $motoristasTotais["esperaIndenizada"],
                        $motoristasTotais["saldoAnterior"],
                        $motoristasTotais["saldoPeriodo"],
                        $motoristasTotais["saldoFinal"]
                    ];
                    $nomeTitulos = [
                        "motorista"         => "Matrícula",
                        "empresaNome"       => "Nome",
                    ...
                    $valorTotais = [
                        $empresas["EmprTotalJorPrev"],
                        (($empresas["EmprTotalJorEfe"] == "00:00")? "": $empresas["EmprTotalJorEfe"]),
                        (($empresas["EmprTotalHE50"] == "00:00")? "": $empresas["EmprTotalHE50"]),
                        (($empresas["EmprTotalHE100"] == "00:00")? "": $empresas["EmprTotalHE100"]),
                        (($empresas["EmprTotalAdicNot"] == "00:00")? "": $empresas["EmprTotalAdicNot"]),
                        (($empresas["EmprTotalEspInd"] == "00:00")? "": $empresas["EmprTotalEspInd"]),
                        (($empresas["EmprTotalSaldoAnter"] == "00:00")? "": $empresas["EmprTotalSaldoAnter"]),
                        (($empresas["EmprTotalSaldoPeriodo"] == "00:00")? "": $empresas["EmprTotalSaldoPeriodo"]),
                        (($empresas["EmprTotalSaldoFinal"] == "00:00")? "": $empresas["EmprTotalSaldoFinal"])
                    ];
                    $nomeTitulos = [
            ...
            $performance = porcentagemSaldo(count($empresasTotais), $quantMeta, $quantNega, $quantPosi);
        }
