<?php
	...
	function buscar() {
		...
		}elseif ($_POST["busca_dataMes"] > date("Y-m")) {
			unset($_POST["acao"]);
			$_POST["errorFields"][] = "busca_dataMes";
		...
		$totais = [
			"jornadaSemRegistro" => 0, "refeicaoSemRegistro" => 0,
			"refeicao1h" => 0, "refeicao2h" => 0,
			"esperaAberto" => 0, "descansoAberto" => 0, "repousoAberto" => 0, "jornadaAberto" => 0,
			"jornadaExedida" => 0,
			"mdcDescanso" => 0,
			"intersticio" => 0
		];
		...
		if (!empty($_POST["acao"]) && $_POST["acao"] == "buscar()") {
			if(empty($_POST["empresa"])){
				dd(["Não tem empresa selecionada.", $_POST]);
			}	
			...
			$motoristas = $ajudantes = $funcionarios = $totalJsonComTudoZero = $totalDiasNaoCFuncionario = 0;
			$totaisFuncionario = [];
			$totaisFuncionario2 = [];
			...
			foreach($empresas as $empresa){
				...
				$path = "./arquivos/nao_conformidade_juridica/{$_POST["busca_dataMes"]}/{$empresa["empr_nb_id"]}/{$pastaArquivo}";

				if (!is_dir($path)) {
					set_status("Arquivo de painel não encontrado. Clique 'Atualizar Painel' para criar um.");
					continue;
				}
				...
				$totalizadores = [
					"refeicao" => 0, "jornadaEfetiva" => 0, "espera" => 0, "descanso" => 0, "repouso" => 0, "jornada" => 0,
					"intersticioInferior" => 0, "intersticioSuperior" => 0,
					"jornadaExcedido10h" => 0, "jornadaExcedido12h" => 0,
					"mdc" => 0, "mdcDescanso30m5h" => 0,
					"refeicaoSemRegistro" => 0, "refeicao1h" => 0, "refeicao2h" => 0,
					"faltaJustificada" => 0, "falta" => 0
				];
				...
				foreach ($arquivos as &$arquivo) {
					...
					if ($json["ocupacao"] === "Motorista") {
						$motoristas++;
					}elseif ($json["ocupacao"] === "Ajudante") {
						$ajudantes++;
					}elseif ($json["ocupacao"] === "Funcionário") {
						$funcionarios++;
					}
				...
				$porcentagemFun = ($totalJsonComTudoZero/($motoristas+$ajudantes+$funcionarios))*100;
	...
	function atualizarPainel(){
		...
		conferirCamposObrig(["busca_dataMes" => "Mês"], $_POST);
		...
		$empresas = mysqli_fetch_all(query(
			"SELECT empr_nb_id FROM empresa WHERE empr_tx_status = 'ativo'
				".(!empty($_POST["empresa"])? "AND empr_nb_id = '{$_POST["empresa"]}'": "")."
			;"
		), MYSQLI_ASSOC);
		foreach($empresas as $empresa){
			relatorio_nao_conformidade_juridica($empresa["empr_nb_id"]);
		}
		...
		echo
			...
			<script>
				function setAndSubmit(empresa){
					document.myForm.acao.value = 'buscar()';
	...
	function index() {
		...
		$buttons = [
			...
			$botao_imprimir,
			$botao_volta,
			botao("Atualizar Painel", "atualizarPainel()", "", "", "", "", "btn btn-warning"),
		];
