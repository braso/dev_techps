<?php
	...
	function calcJorPre(string $jornadaPrevistaOriginal, bool $ehFeriado, $abono = null): string{
		...
		if(!empty($abono)){
		    ...
		    if($abono > $jornadaPrevista){
			    ...
			    $jornadaPrevista = operarHorarios([$jornadaPrevista, $abono], "-");
		    }
		}

		return $jornadaPrevista;
	}
	...
	function diaDetalhePonto(array $motorista, string $data, array $pontosDia = [], $jornadaPrevista = "", $stringFeriado = ""): array{
		...
		if(empty($pontosDia)){
			$pontosDia = mysqli_fetch_all(query(
				"SELECT * FROM ponto
					...
					ORDER BY pont_tx_data ASC, pont_nb_id ASC;"
		...
		if(in_array($motorista["enti_tx_ocupacao"], ["Ajudante", "Motorista"])){
			...
			$aRetorno = reordenarArrayPorChaves($aRetorno, [
				"data",
				"diaSemana",
				"inicioJornada",
				"inicioRefeicao",
		...
		//JORNADA PREVISTA{
			...
			$aRetorno["jornadaPrevista"][0] = $jornadaPrevista;
			if(!empty($abonos)){
				$warning = 
					...
					."</a>&nbsp;"
				;
				$aRetorno["jornadaPrevista"] = [$aRetorno["jornadaPrevista"][0], $warning];
			}
		//}
		//CASO NÃO HAJA PONTOS{
			if(empty($pontosDia)){
				$aRetorno["diffSaldo"][0] = getSaldoDiario($jornadaPrevista, "00:00");

				if(
					$aRetorno["jornadaPrevista"][0] != "00:00"
					&& empty($abonos)
					&& !(date("w", strtotime($data)) == "0" && !empty($motorista["para_tx_jornadaDomingoFacultativo"]))
				){
				...
				if(date("w", strtotime($data)) == "0" && !empty($motorista["para_tx_jornadaDomingoFacultativo"])){
					$aRetorno["jornadaPrevista"][0] = $motorista["para_tx_jornadaDomingoFacultativo"];
					$aRetorno["jornadaPrevista"][] = "<a><i style='color:green;' title='Facultativo' class='color_green fa fa-info-circle'></i></a>";
				}
		...
		if(!empty($pontosDia) && $pontosDia[count($pontosDia)-1]["pont_tx_tipo"] != "2"){//Se o último registro do dia != fim de jornada, significa que há uma jornada aberta que seguiu para os dias seguintes
			query(
				"SET @dataProxFim = (
					SELECT pont_tx_data FROM ponto
		...
		$aRetorno["diffJornada"] = [$diffJornada, $registros["jornadaCompleto"]["icone"]];
		...
		//IGNORAR INTERVALOS E CAMPOS{
			...
			foreach(["refeicao", "espera", "descanso", "repouso"] as $campo){
				if($motorista["enti_tx_ocupacao"] == "Funcionário" && in_array($campo, $intervalosIgnoradosPorFuncionario)){
		...
		//REPOUSO POR ESPERA{
			if(in_array($motorista["enti_tx_ocupacao"], ["Ajudante", "Motorista"])){
				...
				if(!empty($repousosPorEspera["pares"])){
					...
					$repousosPorEspera["icone"] = montarIconeIntervalo($repousosPorEspera["pares"], "color_greenLight fa fa-info-circle", "#00ff00");
		...
		//Converter os totalIntervalo de DateTime para string{
			$totaisKeys = ["refeicaoCompleto", "descansoCompleto", "repousoCompleto", "esperaCompleto", "repousoPorEspera"];
			foreach($totaisKeys as $campo){
		...
		$aRetorno["diffRefeicao"] = [$registros["refeicaoCompleto"]["totalIntervalo"], $registros["refeicaoCompleto"]["icone"]];
		$aRetorno["diffDescanso"] = [$registros["descansoCompleto"]["totalIntervalo"], $registros["descansoCompleto"]["icone"]];
		
		if(in_array($motorista["enti_tx_ocupacao"], ["Ajudante", "Motorista"])){
			$aRetorno["diffEspera"]   = [$registros["esperaCompleto"]["totalIntervalo"], $registros["esperaCompleto"]["icone"]];
			$aRetorno["diffRepouso"]  = [$registros["repousoCompleto"]["totalIntervalo"], $registros["repousoCompleto"]["icone"]];
		}
		...
		//CÁLCULO DE INTERSTÍCIO{
			if(!empty($registros["inicioJornada"])){
				...
				if(!empty($ultimoFimJornada)){
					...
					$aRetorno["intersticio"] = [$totalIntersticio, $icone];
				}else{
					$aRetorno["intersticio"][0] = "00:00";
				}
			}
		//}

		//CALCULO SALDO DIÁRIO{
			$aRetorno["diffSaldo"][0] = getSaldoDiario($jornadaPrevista, $jornadaEfetiva);

			if(date("w", strtotime($data)) == "0" && !empty($motorista["para_tx_jornadaDomingoFacultativo"])){
				if($aRetorno["diffSaldo"][0] == "-"){
					$aRetorno["diffSaldo"][0] = "00:00";
				}
			}
		//}
		...
		//CALCULO ESPERA INDENIZADA{
			if(in_array($motorista["enti_tx_ocupacao"], ["Ajudante", "Motorista"])){
				if($motorista["para_tx_adi5322"] == "sim"){
					$aRetorno["esperaIndenizada"][0] = "00:00";
				}else{
					...
					//Compensar com o intervalo de espera caso o saldo diário esteja negativo{
						if($aRetorno["diffSaldo"][0] == "-"){
							if($intervaloEsp > substr($aRetorno["diffSaldo"][0], 1)){
								$transferir = substr($aRetorno["diffSaldo"][0], 1);
							}else{
								$transferir = $intervaloEsp;
							}
							$intervaloEsp = operarHorarios([$intervaloEsp, $transferir], "-");
							$aRetorno["diffSaldo"][0] = operarHorarios([$aRetorno["diffSaldo"][0], $transferir], "+");
						}
					//}
	
					if($indenizarEspera){
						$aRetorno["esperaIndenizada"][0] = $intervaloEsp;
					}
				}
			}
		//}
		...
		//HORAS EXTRAS{
			if($aRetorno["diffSaldo"][0] != "-"){ 	//Se o saldo for positivo
				$ehDomingoFacultativo = (date("w", strtotime($data)) == "0" && !empty($motorista["para_tx_jornadaDomingoFacultativo"]));
				if(($aRetorno["jornadaPrevista"][0] == "00:00" || $ehDomingoFacultativo)){
					$aRetorno["he100"][0] = $aRetorno["diffSaldo"][0];
					$aRetorno["he50"][0] = "00:00";
					if($ehDomingoFacultativo){
						$aRetorno["diffSaldo"][0] = "00:00";
					}
				}else{
					if(!empty($motorista["para_tx_maxHESemanalDiario"]) 
						&& $motorista["para_tx_maxHESemanalDiario"] != "00:00" 
							&& $aRetorno["diffSaldo"][0] >= $motorista["para_tx_maxHESemanalDiario"]
					){// saldo diário >= limite de horas extras 100%
						$aRetorno["he100"][0] = operarHorarios([$aRetorno["diffSaldo"][0], $motorista["para_tx_maxHESemanalDiario"]], "-");
					}else{
						$aRetorno["he100"][0] = "00:00";
					}
					$aRetorno["he50"][0] = operarHorarios([$aRetorno["diffSaldo"][0], $aRetorno["he100"][0]], "-");
				}
			}
		//}
		...
		//ALERTAS{
			if(empty($registros["inicioJornada"][0]) && $aRetorno["jornadaPrevista"][0] != "00:00"){
				$aRetorno["inicioJornada"][] 	= "<a><i style='color:red;' title='Batida início de jornada não registrada!' class='color_red fa fa-warning'></i></a>";
			}
			if($fezJorMinima || count($registros["inicioJornada"]) > 0){
				if(!isset($registros["fimJornada"][0]) || $registros["fimJornada"][0] == ""){
					$aRetorno["fimJornada"][] 	  = "<a><i style='color:red;' title='Batida fim de jornada não registrada!' class='color_red fa fa-warning'></i></a>";
				}
				...
				//01:00 DE REFEICAO{
					...
					if($maiorRefeicao > "02:00"){
						$avisoRefeicao = "<a><i style='color:orange;' title='Refeição com tempo máximo de 02:00h não respeitado.' class='color_orange fa fa-info-circle'></i></a>";
					}elseif($dtJornada > $dtJornadaMinima && $maiorRefeicao < '01:00'){
						$avisoRefeicao = "<a><i style='color:red;' title='Refeição ininterrupta maior do que 01:00h não respeitado.' class='color_red fa fa-warning'></i></a>";
					}
				//}

				// if((!isset($registros["inicioRefeicao"][0]) || empty($aRetorno["inicioRefeicao"][0])) && $jornadaEfetiva > "06:00"){
				// 	$aRetorno["inicioRefeicao"][] = "<a><i style='color:red;' title='Batida início de refeição não registrada!' class='color_red fa fa-warning'></i></a>";
				// }else{
				// 	$aRetorno["inicioRefeicao"][] = $avisoRefeicao;
				// }

				// if((!isset($registros["fimRefeicao"][0]) || empty($aRetorno["fimRefeicao"][0])) && ($jornadaEfetiva > "06:00")){
				// 	$aRetorno["fimRefeicao"][] 	  = "<a><i style='color:red;' title='Batida fim de refeição não registrada!' class='color_red fa fa-warning'></i></a>";
				// }else{
				// 	$aRetorno["fimRefeicao"][] = $avisoRefeicao;
				// }
				if(!empty($avisoRefeicao)){
					$aRetorno["diffRefeicao"][] = $avisoRefeicao;
				}
			}
		//}
		...
		foreach(["inicioJornada", "fimJornada", "inicioRefeicao", "fimRefeicao"] as $campo){
			if(count($registros[$campo]) > 0 && !empty($registros[$campo][0])){
				$aRetorno[$campo] = array_merge($aRetorno[$campo], $registros[$campo]);
			}
		}
		...
		if($saldo > 0){
			$aRetorno["diffSaldo"][0] = "<b>{$aRetorno["diffSaldo"][0]}</b>";
		}
			...
			foreach(["inicioJornada", "fimJornada", "inicioRefeicao", "fimRefeicao"] as $tipo){
				...
				if(count($aRetorno[$tipo]) > 0){
					foreach($aRetorno[$tipo] as $ponto){
						$ponto = is_array($ponto)? $ponto[0]: $ponto;
				...
				if(!empty($pontos)){
					$dataDia = DateTime::createFromFormat("d/m/Y H:i:s", $aRetorno["data"][0]." 00:00:00");
		...
		//Converter array em string{
			...
			$getStringPontosColuna = function (array $pontos): string{
			...
			foreach(["inicioJornada", "fimJornada", "inicioRefeicao", "fimRefeicao"] as $tipo){
				$aRetorno[$tipo][0] = $getStringPontosColuna($aRetorno[$tipo]);
			}
		//}
		...
		if(count($endossos) > 0){
			...
			for($f = 1; $f < count($endossos); $f++){
				...
				foreach($endossos[$f]["totalResumo"] as $key => $value){
					if(in_array($key, ["saldoAnterior", "saldoBruto", "saldoFinal", "descFaltas"])){
						continue;
					}
					$endossoCompleto["totalResumo"][$key] = operarHorarios([($endossoCompleto["totalResumo"][$key]?? "00:00"), $value], "+");
				}
			}
			if(count($endossos) > 1){
	...
	function montarIconeIntervalo(array $pares, string $classe = "color_green fa fa-info-circle", string $cor = "green"): string{
		$icone = "";
		$tooltip = "";
	...
	function pegarSqlDia(string $matricula, DateTime $data, array $cols): string{
		...
		$sql = 
			"SELECT DISTINCT pont_nb_id, ".implode(",", $cols)." FROM ponto"
				...
				." LEFT JOIN motivo ON ponto.pont_nb_motivo = motivo.moti_nb_id"
				." LEFT JOIN endosso ON entidade.enti_nb_id = endosso.endo_nb_entidade AND endo_tx_status = 'ativo' AND '{$data->format("Y-m-d")}' BETWEEN endo_tx_de AND endo_tx_ate"
				." WHERE {$condicoesPontoBasicas}"
	...
	function verificaLimiteTempo(string $tempoEfetuado, string $limite): array{
		...
		if($datetime1 > $datetime2){
			return [$tempoEfetuado, "<a style='white-space: nowrap;'><i style='color:orange;' title='Tempo excedido de ".$limite."' class='color_orange fa fa-warning'></i></a>&nbsp;"];
		}else{
			return [$tempoEfetuado];
		}
	}

	function verificarAlertaMDC(array $intervalos = []): array{
		...
		if($mdc > "05:30"){
			return [
				...
				"<a style='white-space: nowrap;'>".
					"<i style='color:orange;' title='{$baseErrMsg}' class='color_orange fa fa-warning'></i>".
				"</a>".
				"&nbsp;"
			];
		}
		...
		for($f = 0; $f < count($intervalos); $f++){
			...
			if($somaTempoAtivo > $tempoTrabalho && $somaTempoDescanso < $tempoDescanso){
				$res = [
					$mdc, 
					"<a style='white-space: nowrap;'>".
						"<i style='color:orange;' title='{$baseErrMsg}\n\nDirigido: ".$somaTempoAtivo."\nDescansado: ".$somaTempoDescanso."' class='color_orange fa fa-warning'></i>".
					"</a>".
					"&nbsp;"
				];
				$f = count($intervalos);
	...
	function verificaTolerancia(string $saldoDiario, string $data, $idMotorista){
		...
		$endossado = mysqli_fetch_all(
			query(
				"SELECT * FROM endosso 
					JOIN entidade ON endo_nb_entidade = enti_nb_id
	...
	function setTotalResumo(array $keys): array{
		$totalResumo = [];
		foreach($keys as $key){
	...
	function somarTotais(array &$totalResumo, array &$rows){
		$colunasASomar = array_keys(array_slice($rows[0], 7));
		foreach($rows as $row){
		...
		if($totalResumo["diffSaldo"][0] != "-" && $totalResumo["diffSaldo"] != "00:00"){
			$totalResumo["diffSaldo"] = "<b>{$totalResumo["diffSaldo"]}</b>";
		}
	}