...
function assinatura_preview_status(): array {
    if(extension_loaded("imagick")){
        return ["ok" => true, "engine" => "imagick", "hint" => null];
    }
    if(canExec()){
        $gs = findCommand(["gswin64c", "gswin32c", "gs"]);
        if($gs){
            return ["ok" => true, "engine" => "ghostscript", "hint" => null];
        }
        return ["ok" => false, "engine" => null, "hint" => "Ghostscript não encontrado no servidor."];
    }
    return ["ok" => false, "engine" => null, "hint" => "Função exec() bloqueada no servidor."];
}
        ...
        if($ext !== "pdf"){
            ...
            if(!is_dir($dirTmp)){
                if(!@mkdir($dirTmp, 0777, true)){
                    $erroUploadPdf = "Sem permissão para criar a pasta de uploads temporários.";
                }
            }
            if($erroUploadPdf !== ""){
            } elseif(!is_writable($dirTmp)){
                $erroUploadPdf = "Sem permissão de escrita na pasta de uploads temporários.";
            }
            $token = gerarTokenPdf();
            ...
            if($erroUploadPdf !== ""){
                ...
                if($paginas <= 0){
                    ...
                    if($previewOk === 0){
                        $st = assinatura_preview_status();
                        $hint = trim(strval($st["hint"] ?? ""));
                        if($hint === ""){
                            $hint = "Servidor sem suporte para gerar miniaturas (Imagick/Ghostscript).";
                        }
                        $_SESSION["pdf_split_tokens"][$token]["thumb_error"] = $hint;
                    }
                    $cpfToFuncionario = [];
                            ...
                            <?php
                                ...
                                if(session_status() === PHP_SESSION_ACTIVE){
                                    ...
                                    if(is_array($tokenInfo)){
                                        $thumbError = trim(strval($tokenInfo["thumb_error"] ?? ""));
                                    }
                                }
                            ?>
                            <?php if($thumbError): ?>
                                        ...
                                        <div class="border border-gray-200 rounded-xl p-4 flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                                            ...
                                            <div class="flex items-start gap-3">
                                                <div class="w-20">
                                                    ...
                                                    <?php else: ?>
                                                        <?php $pdfRel = "uploads/tmp/" . $tokenPdf . ".pdf"; ?>
                                                        <a href="<?php echo htmlspecialchars($pdfRel . "#page=" . $p); ?>" target="_blank" rel="noopener" class="w-20 h-28 flex items-center justify-center bg-gray-50 border border-gray-200 rounded-md text-[10px] text-blue-700 text-center px-2 hover:bg-blue-50 hover:border-blue-200">
                                                            Abrir página
                                                        </a>
                                                    <?php endif; ?>
                                                </div>
                ...
                <div id="assinaturaStep" style="<?php echo $modo_upload ? 'display:none;' : 'display:flex;'; ?>" class="flex flex-col lg:flex-row h-full">
                    ...
                    <div class="lg:w-1/2 p-6 lg:p-8 flex flex-col justify-center bg-white">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <label class="flex items-start gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-white cursor-pointer">
                                        <input type="radio" name="documento_assinar" value="sim" checked class="mt-1 h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500">
                                        <div>
                                            <div class="text-sm font-semibold text-gray-800">Sim</div>
                                            <div class="text-xs text-gray-500">Enviar para assinatura eletrônica.</div>
                                        </div>
                                    </label>
                                    <label class="flex items-start gap-3 px-4 py-3 rounded-xl border border-gray-200 bg-white cursor-pointer">
                                        <input type="radio" name="documento_assinar" value="nao" class="mt-1 h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500">
                                        <div>
                                            <div class="text-sm font-semibold text-gray-800">Não</div>
                                            <div class="text-xs text-gray-500">Enviar por e-mail (sem assinatura).</div>
                                        </div>
                                    </label>
                                </div>
                            </div>
                            ...
                            <div id="avulso_signatario_wrap_email">
                                <label id="avulso_label_email" for="email" class="block text-sm font-medium text-gray-700 mb-1">E-mail do Signatário</label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            ...
                            <div id="avulso_signatario_info" class="bg-blue-50 rounded-lg p-4 border border-blue-100 mt-4">
                                <p id="avulso_info_texto" class="text-blue-700 text-xs text-justify leading-relaxed">
                            ...
                            <button type="submit" id="btnEnviar" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-bold text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all transform hover:-translate-y-0.5">
                                <i class="fas fa-paper-plane mr-2 text-lg"></i> <span id="btnEnviarLabel">Enviar para Assinatura</span>
                            </button>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        const btnEnviarLabel = document.getElementById("btnEnviarLabel");
        const tituloDados = document.getElementById("avulso_titulo_dados");
        const subTituloDados = document.getElementById("avulso_subtitulo_dados");
        ...
        const atualizarAvulso = () => {
            ...
            if(assinar !== "sim"){
                ...
                if(tituloDados){ tituloDados.textContent = "Dados do Destinatário"; }
                if(subTituloDados){ subTituloDados.textContent = "Informe quem receberá o documento por e-mail."; }
                if(labelNome){ labelNome.textContent = "Nome do Destinatário"; }
                if(labelEmail){ labelEmail.textContent = "E-mail do Destinatário"; }
                if(infoTexto){
                    infoTexto.innerHTML = '<i class="fas fa-info-circle mr-1"></i>O destinatário receberá o PDF por e-mail (sem precisar assinar).';
                }
                if(btnEnviarLabel){ btnEnviarLabel.textContent = "Enviar Documento"; }
                return;
            }
            icpAvulso.disabled = false;
            icpInfoAvulso.classList.toggle("hidden", !icpAvulso.checked);
            if(tituloDados){ tituloDados.textContent = "Dados do Signatário"; }
            if(subTituloDados){ subTituloDados.textContent = "Informe quem deverá assinar este documento."; }
            if(labelNome){ labelNome.textContent = "Nome do Signatário"; }
            if(labelEmail){ labelEmail.textContent = "E-mail do Signatário"; }
            if(infoTexto){
                infoTexto.innerHTML = '<i class="fas fa-info-circle mr-1"></i>O signatário receberá um e-mail com um link único e seguro para realizar a assinatura eletrônica do documento.';
            }
            if(btnEnviarLabel){ btnEnviarLabel.textContent = "Enviar para Assinatura"; }
        };
        if(icpAvulso){
