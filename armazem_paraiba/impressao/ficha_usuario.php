...
$usuario = mysqli_fetch_assoc(
    query(
        "SELECT user.*, entidade.enti_tx_matricula FROM user 
            LEFT JOIN entidade ON user_nb_entidade = enti_nb_id
            WHERE user_nb_id = {$_POST["id_usuario"]};"
    ), 
    MYSQLI_ASSOC
);

$empresa = $usuario["user_nb_empresa"]? carregar("empresa", $usuario["user_nb_empresa"]) : null;
$cidade  = $usuario["user_nb_cidade"]? carregar("cidade", $usuario["user_nb_cidade"]) : null;
...
$pdf->SetX($x_col1_start);
createField($pdf, "Matricula", $usuario["enti_tx_matricula"], $col_data_width);
$pdf->SetX($x_col2_start);
