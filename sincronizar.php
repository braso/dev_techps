...
$conn1 = new PDO("mysql:host=localhost;dbname=techpsjornada_opafrutas", "techpsjornada_opafrutas", "BpV,M%gSv{^*nyv!aA", [
    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION
]);

$conn2 = new PDO("mysql:host=localhost;dbname=techpsjornada_opafrutas_dev", "techpsjornada_opafrutas_dev", "^,B)g0[~r.0hy!yCr,", [
    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION
]);
...
$deleteQuery = "
    DELETE FROM ponto 
    WHERE DATE_FORMAT(pont_tx_data, '%Y-%m') >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m');
";
$conn2->exec($deleteQuery);

// Buscar dados do Banco 1 apenas para os dois últimos meses
$query = $conn1->prepare("
    SELECT * FROM ponto 
    WHERE DATE_FORMAT(pont_tx_data, '%Y-%m') >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 2 MONTH), '%Y-%m');
");
$query->execute();
...
if (!empty($dados)) {
    ...
    $stmt = $conn2->prepare("INSERT INTO ponto ($colunasString) VALUES ($placeholders)");
